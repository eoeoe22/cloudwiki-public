...
`|` 파이프 기호를 이용해 링크와 표시 텍스트를 조절할 수 있습니다. [* 표시명 변경은 표 내부에서 사용할 수 없습니다.]
...
`#` 뒤에 목차 번호를 붙이면 링크 클릭 시 문서 로드 후 해당 목차 위치로 즉시 스크롤됩니다.
문서 슬러그는 `#` 문자를 포함할 수 없으므로, `#` 이후는 항상 앵커로 해석됩니다.
목차 번호가 잘못된 형식이면 앵커는 무시되고 문서만 이동합니다.
