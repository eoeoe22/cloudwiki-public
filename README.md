...
Cloudflare 대시보드의 보안 메뉴의 Turnstile 에서 **위젯 추가** 를 클릭하고, 이름을 설정한 뒤, **호스트 이름 추가** 를 통해 사용할 도메인을 입력한 후, 클라이언트 ID, 시크릿을 발급받고, 복사해 저장합니다.
wrangler.toml에 적힌 주석을 참고해 나머지 설정값들도 적절히 설정합니다.
(구글 Oauth 클라이언트 시크릿, Turnstile 클라이언트 시크릿은 wrangler.toml에 직접 저장해도 작동상 문제는 없으나, 보안을 위해 Workers 대시보드의 설정 탭에서 **비밀** 환경변수로 설정하는것을 권장합니다.)
