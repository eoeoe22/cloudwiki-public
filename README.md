...
* 사용자 인증 및 권한 관리: Google OAuth를 통한 로그인 기능과 일반 사용자, 토론 관리자, 관리자, 최고 관리자 등 세분화된 권한을 지원합니다.
* 리비전 및 변경 이력: 위키 문서의 모든 편집 이력을 저장하여 이전 버전으로 되돌리거나(Revert), 두 리비전 간의 차이(Diff)를 비교할 수 있습니다.
...
* 입력한 내용이 문서 열람시와 동일한 형태로 프리뷰 렌더링되며, 화면이 충분히 넓은 PC 환경에서 한눈에 미리보기 렌더링을 볼수 있는 스플릿 뷰를 제공합니다.
* 위키 내부 링크, 틀, 커스텀 색상코드, 아이콘 삽입 문법 입력을 시도시 클릭 또는 키보드 화살표+엔터로 간편하게 조작 가능한 자동완성 모달이 커서 옆에 표시됩니다.
* 실시간 프리뷰 에디터: 작성한 문서 내용이 실제 열보과 동일한 형태로 실시간 렌더링됩니다.
* 미디어 업로드: Cloudflare R2 버킷을 이용해 이미지 등의 미디어 파일을 안정적으로 업로드하고 관리합니다.
* 문서 리다이렉트 및 별칭: 여러 검색어가 한가지 문서로 향하도록 하기 위한 리디렉션 전용 문서를 생성할수도 있습니다.
* 커스터마이징: 푸터, 사이드바 영역, 위키 이름 및 로고 브랜딩을 자유롭게 커스터마이징 할 수 있습니다.
...
* 랜덤 문서로 이동하기 버튼을 사용할수 있습니다.
* MCP 서버 지원: AI 모델이 위키 데이터를 직접 탐색하고 읽을 수 있도록 MCP를 지원합니다. 제목 검색, 본문 전문 검색, 목차 추출, 섹션별 읽기 등 AI 최적화된 도구 세트를 제공합니다. MCP를 통해 문서를 읽을때 불필요한 전용 확장 문법 (표 색상 지정 등)은 자동으로 제거되어 전달됩니다.
...
│   ├── routes/         # API 라우트 핸들러 (auth, wiki, search, admin, mcp 등)
│   ├── utils/          # 유틸리티 함수 (slug 정규화, AI 파서, 권한 검사 등)
...
CloudWiki는 AI 에이전트가 위키의 지식을 직접 활용할 수 있도록 MCP 서버 기능을 내장하고 있습니다. AI가 브라우저 조작 없이 위키 문서를 검색하고, 내용을 읽고, 구조를 파악할 수 있습니다.

### 지원 도구
* `search_title`: 위키 문서 제목 검색
* `search_fts`: 위키 문서 본문 전문 검색 (SQLite FTS5 활용)
* `get_toc`: 문서의 목차(Heading) 구조 추출
* `read_document`: 문서 전체 내용을 AI가 이해하기 쉬운 형태로 최적화하여 읽기 (틀 트랜스클루전 처리 포함)
* `read_section`: 특정 섹션(목차)의 내용만 정밀하게 읽기

### 연결 방법
관리자 설정에서 MCP 기능을 활성화 후, AI 애플리케이션의 MCP 서버 설정에 URL을 입력합니다.
