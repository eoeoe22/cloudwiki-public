...
// ── 헬퍼: 사이드바 URL 안전성 검사 (javascript: 스킴 차단) ──
...
async function renderHtml(c: Context<Env>, targetHtmlPath: string, pageData: Record<string, any> = {}): Promise<Response> {
            ...
            if (Array.isArray(config)) {
                for (const item of config) {
                    if (item.type === 'header') {
                        ...
                        customSidebarHtml += `<li><hr class="dropdown-divider my-2" style="border-color: var(--wiki-text);"></li>`;
                    }
                }
            }
        }
    ...
    // Header와 Sidebar HTML 로드 및 브랜딩 적용
    let headerHtml = '';
    let sidebarHtml = '';
