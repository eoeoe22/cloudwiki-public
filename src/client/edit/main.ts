...
// MCP 드래프트의 requested_lock 값(0|1|null). 본문/카테고리/제목 변경이 전혀 없는 lock-only
// 드래프트가 로드된 경우, savePage 가 hasMeaningfulChanges()==false 로 단축 종료하기 전에
// 이 값을 보고 PUT 없이 /resolve 만 호출해 서버측이 잠금을 적용하도록 한다.
let mcpRequestedLock: number | null = null;
...
async function loadMcpSubmissionIntoEditor(submissionId: number): Promise<boolean> {
    ...
    // 잠금 변경 요청(requested_lock) 은 서버측 mcp-submissions /resolve 가 후처리로 적용한다.
    // 에디터 안에서는 잠금 토글이 제거되었으므로 클라이언트 측 사전 적용은 더 이상 수행하지 않고,
    // 다만 lock-only 드래프트(본문 변경 없음) 가 hasMeaningfulChanges() 단축 종료에 막혀
    // /resolve 까지 도달하지 못하는 케이스만 모듈 상태로 기억해 savePage 가 우회 처리한다.
    mcpRequestedLock = (detail.requested_lock === 0 || detail.requested_lock === 1)
        ? detail.requested_lock
        : null;
...
async function savePage() {
    ...
    // is_locked / is_private 는 에디터 폼에서 제거됨 — 권한 관리 모달이 단건 전용 엔드포인트로 처리.
    // PUT /api/w/:slug 는 두 키가 누락되면 기존 값을 그대로 유지한다 (wiki.ts 의 ?? fallback).
    ...
    // 단, 활성 MCP 드래프트가 lock-only 변경(본문/카테고리/제목 동일 + requested_lock 만 차이)이면
    // PUT 으로 리비전을 만들지 않고 곧장 /resolve 만 호출해 서버측이 lock 을 적용·정리하도록 한다.
    if (!hasMeaningfulChanges()) {
        const origLocked = originalPageMeta ? (originalPageMeta.is_locked ? 1 : 0) : 0;
        const isLockOnlyMcp = mcpSubmissionId !== null
            && mcpRequestedLock !== null
            && mcpRequestedLock !== origLocked;
        if (!isLockOnlyMcp) {
            window.Swal.fire({
                icon: 'info',
                title: '변경된 내용이 없습니다',
        ...
        // Lock-only MCP 드래프트 단축 경로
        const saveBtnEl = document.getElementById('saveBtn') as HTMLButtonElement | null;
        if (saveInProgress) return;
        saveInProgress = true;
        if (saveBtnEl) {
            saveBtnEl.disabled = true;
            saveBtnEl.innerHTML = '<span class="spinner-border spinner-border-sm"></span> 적용 중...';
        }
        try {
            ...
            if (!resolveRes.ok) {
                ...
                throw new Error(errBody?.error || `잠금 적용 실패 (HTTP ${resolveRes.status})`);
            }
            const resolveData = await resolveRes.json().catch(() => ({}));
            mcpSubmissionId = null;
            mcpRequestedLock = null;
            // originalPageMeta.is_locked 도 새 값으로 갱신해 이후 hasMeaningfulChanges 가 안정.
            if (originalPageMeta && typeof resolveData?.lock_applied?.to === 'number') {
                originalPageMeta.is_locked = resolveData.lock_applied.to;
            }
            await window.Swal.fire({
                icon: 'success',
                title: '잠금 변경 적용됨',
            ...
            window.Swal.fire({ icon: 'error', title: '실패', text: String(e?.message || e) });
            if (saveBtnEl) {
                saveBtnEl.disabled = false;
                saveBtnEl.innerHTML = '<i class="mdi mdi-check"></i> 저장';
            }
