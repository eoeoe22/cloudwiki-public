...
/**
 * GET /wiki/templates
 * 템플릿 목록 (slug가 'template:'으로 시작하는 문서들)
 */
wiki.get('/wiki/templates', async (c) => {
    ...
    if (q) {
        const { results } = await db
            .prepare("SELECT slug, title FROM pages WHERE (slug LIKE 'template:%' OR slug LIKE '틀:%' OR slug LIKE '템플릿:%') AND title LIKE ? AND deleted_at IS NULL ORDER BY created_at DESC")
            .bind(`%${q}%`)
            .all();
        return c.json({ templates: results });
    } else {
        const { results } = await db
            .prepare("SELECT slug, title FROM pages WHERE (slug LIKE 'template:%' OR slug LIKE '틀:%' OR slug LIKE '템플릿:%') AND deleted_at IS NULL ORDER BY created_at DESC LIMIT 10")
            .all();
        return c.json({ templates: results });
    }
});
...
wiki.get('/wiki/:slug/revisions/:id/diff', async (c) => {
    ...
    const revision = await db
        .prepare(
            `SELECT r.id, r.content, r.page_id, r.created_at
       FROM revisions r
       JOIN pages p ON r.page_id = p.id
       WHERE r.id = ? AND p.slug = ?`
        )
 ...
 * - 기존 문서 이름은 새로운 문서 이름으로의 넘겨주기(Redirect)로 자동 등록
...
wiki.post('/wiki/:slug/move', requireAuth, async (c) => {
    ...
    // Transaction likely needed but D1 basic batch is for writes.
    // 1. Create Redirect: Old Slug -> Page ID (which will have New Slug soon)
    await db.prepare('INSERT INTO redirects (source_slug, target_page_id) VALUES (?, ?)')
        .bind(currentSlug, page.id)
        .run();

    // 2. Update Page Slug and Title
