...
/** 한 번에 검색할 수 있는 문서 최대 개수(삭제/이동 선택 상한과 공유). */
const BULK_MANAGE_MAX = 500;

/**
 * 한 번의 요청(= Worker 1회 호출)으로 이동(제목 변경)할 수 있는 문서 최대 개수.
 * 이동은 항목마다 다수의 D1 쿼리(page/conflict/category 조회 + slug update + 최근변경 + admin_log)
 * 를 수행하므로, D1 의 유료 플랜 호출당 쿼리 한도(1,000)를 넘지 않도록 보수적으로 제한한다.
 * (본 배포는 analytics_engine_datasets 바인딩이 있는 유료 Workers 라 한도는 50 이 아닌 1,000.)
 * 클라이언트는 선택 전체를 이 한도 이하의 청크로 쪼개 여러 번 호출한다(아래 라우트 주석 참조).
 */
const BULK_MOVE_MAX = 50;

/**
 * 역링크 일괄 갱신(update_backlinks=true)을 켠 경우의 요청당 최대 문서 수.
 * 역링크 재작성(rewriteBacklinksForRename)은 소스 1건당 revision SELECT/INSERT + page UPDATE +
 * link 재구축으로, 한 문서가 최대 MAX_BACKLINKS(200) 소스까지 처리하면 단일 이동만으로도
 * ~800 쿼리에 달한다. 따라서 1회 호출에 2건 이상을 묶으면 1,000 한도를 넘길 수 있어,
 * 역링크 경로는 **문서 1건/요청**으로 강제한다(200-백링크 캡 덕에 통상 1건은 한도 내 안전).
 * 클라이언트도 이 경우 청크 크기를 1 로 잡아 문서 수만큼 순차 호출한다.
 *
 * 알려진 한계: 단일 문서가 ~125건 이상의 역링크를 가지면 rewriteBacklinksForRename 의
 * 소스당 statement(rev SELECT/INSERT + page UPDATE + 링크 재구축 ≈ 6~8개)가 1,000 한도에
 * 근접/초과할 수 있다. 단, 이는 본 대량 이동 고유 문제가 아니라 **동일 헬퍼를 쓰는 기존 단건
 * 이동 엔드포인트 `POST /w/:slug/move` 와 완전히 동일한 한계**다(같은 MAX_BACKLINKS=200). 대량
 * 경로를 1건/요청으로 묶었으므로 단건 이동 대비 호출당 위험을 늘리지 않는다. 역링크 재작성
 * 자체를 statement 예산 기준으로 분할/축소하는 것은 공유 헬퍼(단건 이동 포함) 차원의 별도 과제다.
 */
const BULK_MOVE_BACKLINK_MAX = 1;
 ...
 */
adminRoutes.post('/bulk-manage/delete', async (c) => {
    const currentUser = c.get('user')!;
    const rbac = c.get('rbac') as RBAC;
    if (!rbac.can(currentUser.role, '*')) {
        return c.json({ error: '최고 관리자만 사용할 수 있습니다.' }, 403);
    ...
    if (ids.length > BULK_MANAGE_MAX) {
        return c.json({ error: `한 번에 삭제할 수 있는 문서는 ${BULK_MANAGE_MAX}개까지입니다.` }, 400);
    }
...
/**
 * POST /bulk-manage/move
 * 선택한 문서들의 slug 에서 `find` 부분 문자열을 `replace` 로 치환해 일괄 이동(제목 변경)한다.
 * (최고 관리자 전용)
 * body: { ids: number[], find: string, replace: string, update_backlinks?: boolean }
 *
 * - 각 문서의 새 slug = oldSlug.split(find).join(replace) (모든 occurrence 치환).
 *   find 가 등장하지 않아 변경이 없는 문서는 건너뛴다.
 * - 실제 이동은 공유 헬퍼 movePage 에 위임한다 — 단건 이동(POST /w/:slug/move)과 동일한
 *   검증·카테고리 prefix 룰·캐시 무효화·(옵션) 역링크 일괄 재작성을 그대로 사용한다.
 * - 항목별로 독립 처리하며, 충돌/검증/소프트삭제/미발견 실패는 모두 skipped 로 보고해
 *   requested = moved + skipped(+ 변경없음) 이 항상 맞아떨어지도록 한다(성공분은 유지).
 * - 이동은 항목마다 page/conflict/category 조회 + slug update + 최근변경 갱신 + admin_log,
 *   그리고 (옵션) 역링크 재작성(소스 1건당 R2+리비전+update+링크 재구축)으로 D1 쿼리 비용이
 *   매우 크다. D1 은 Worker 호출당 쿼리 수에 한도(유료 1,000/무료 50)가 있으므로 한 요청에서
 *   많은 문서를 돌리면 루프 도중 한도 초과로 부분 실행(이미 이동된 건은 커밋)될 수 있다.
 *   따라서 한 요청은 무백링크 BULK_MOVE_MAX, 역링크 갱신 시 BULK_MOVE_BACKLINK_MAX(=1) 로
 *   제한하고, 클라이언트가 선택 전체를 이 한도 이하의 청크로 쪼개 **여러 번의 독립 호출**로
 *   처리한다(엔드포인트는 청크 단위로 멱등적으로 동작).
 * - update_backlinks=true 인 항목에서 이동은 성공했으나 역링크 재작성만 실패한 경우
 *   (movePage 가 ok=true + backlinks_error 반환), 해당 항목을 backlink_errors 로 보고한다.
 *
 * 주의: 두 문서가 같은 새 slug 로 치환되거나(중복), 새 slug 가 아직 이동되지 않은 다른
 * 선택 문서와 충돌하는 경우(swap/shift)는 충돌로 보고될 수 있다. find/replace 가
 * 보통 새 네임스페이스/접두로 치환하는 용도라 실무에서는 드물다.
 */
adminRoutes.post('/bulk-manage/move', async (c) => {
    const currentUser = c.get('user')!;
    const rbac = c.get('rbac') as RBAC;
    if (!rbac.can(currentUser.role, '*')) {
        return c.json({ error: '최고 관리자만 사용할 수 있습니다.' }, 403);
    ...
    const body = await c.req
        .json<{ ids?: unknown; find?: unknown; replace?: unknown; update_backlinks?: unknown }>()
        .catch(() => ({} as any));
    ...
    // 역링크 갱신 경로는 문서당 D1 비용이 매우 커 1건/요청으로 제한한다(위 상수 주석 참조).
    // 클라이언트가 청크로 쪼개 호출해야 하는 한도이자, 직접 API 오남용 방지용 가드.
    const perRequestMax = updateBacklinks ? BULK_MOVE_BACKLINK_MAX : BULK_MOVE_MAX;
    if (ids.length > perRequestMax) {
        return c.json({
            error: updateBacklinks
                ? `참조 링크 갱신을 켜면 한 번의 요청으로 ${BULK_MOVE_BACKLINK_MAX}개 문서만 이동할 수 있습니다. 선택을 나눠 요청하세요.`
                : `한 번의 요청으로 이동할 수 있는 문서는 ${BULK_MOVE_MAX}개까지입니다. 선택을 나눠 요청하세요.`,
        }, 400);
    }

    // 요청된 모든 id 를 조회(예약 네임스페이스/삭제 여부 포함). D1 100 bind 제한 → 90개씩 청크.
    // 어떤 필터도 걸지 않고 deleted_at 까지 받아와, 미발견/삭제/네임스페이스 등 모든 비대상을
    // skipped 로 명시 보고한다(silent drop 으로 requested>moved 불일치가 생기지 않게).
    const SCAN_CHUNK = 90;
    const byId = new Map<number, { slug: string; deleted_at: number | null }>();
    for (let i = 0; i < ids.length; i += SCAN_CHUNK) {
        const chunkIds = ids.slice(i, i + SCAN_CHUNK);
        const ph = chunkIds.map(() => '?').join(',');
        const { results } = await db
            .prepare(`SELECT id, slug, deleted_at FROM pages WHERE id IN (${ph})`)
            .bind(...chunkIds)
            .all<{ id: number; slug: string; deleted_at: number | null }>();
        for (const r of results) byId.set(r.id, { slug: r.slug, deleted_at: r.deleted_at });
    }

    const moved: { old_slug: string; new_slug: string; backlinks_updated: number; backlinks_error?: string }[] = [];
    const skipped: { slug: string; reason: string }[] = [];
    const backlinkErrors: { slug: string; error: string }[] = [];
    // 이동은 성공했으나 일부 역링크가 갱신되지 않은 항목(>200 캡/R2 읽기 실패/낙관적 락 충돌 등으로
    // rewriteBacklinksForRename 가 throw 없이 skipped/conflicts 로 반환). 부분 실패로 분리 보고한다.
    const backlinkPartials: { slug: string; skipped: number; conflicts: number }[] = [];
    let backlinksUpdated = 0;
    let backlinksSkipped = 0;
    let backlinksConflicts = 0;

    // 미발견/삭제 문서를 먼저 걸러 skipped 에 기록하고, 이동 가능한 대상만 slug ASC 로 처리한다.
    const movable: { slug: string }[] = [];
    for (const id of ids) {
        const row = byId.get(id);
        if (!row) {
            skipped.push({ slug: `#${id}`, reason: '문서를 찾을 수 없습니다.' });
            continue;
        }
        if (row.deleted_at != null) {
            skipped.push({ slug: row.slug, reason: '삭제된 문서는 이동할 수 없습니다.' });
            continue;
        }
        movable.push({ slug: row.slug });
    }
    movable.sort((a, b) => (a.slug < b.slug ? -1 : a.slug > b.slug ? 1 : 0));

    for (const t of movable) {
        const newSlug = t.slug.split(find).join(replace);
        if (newSlug === t.slug) {
            skipped.push({ slug: t.slug, reason: '변경 없음 (찾을 내용 미포함)' });
            continue;
        }
        const outcome = await movePage(c, t.slug, newSlug, currentUser, rbac, { updateBacklinks });
        if (!outcome.ok) {
            skipped.push({ slug: t.slug, reason: outcome.error || '이동 실패' });
            continue;
        }
        ...
        backlinksUpdated += blUpdated;
        backlinksSkipped += blSkipped;
        backlinksConflicts += blConflicts;
        // 이동은 성공했으나 역링크 재작성 전체가 throw 로 실패한 경우(부분 성공)를 별도로 보고한다.
        if (outcome.backlinks_error) {
            backlinkErrors.push({ slug: outcome.new_slug!, error: outcome.backlinks_error });
        }
        // 이동·역링크 호출은 성공했으나 일부 소스만 갱신 못 한 경우(캡/충돌/읽기 실패)도 보고한다.
        if (blSkipped > 0 || blConflicts > 0) {
            backlinkPartials.push({ slug: outcome.new_slug!, skipped: blSkipped, conflicts: blConflicts });
        }
        moved.push({
            old_slug: t.slug,
            new_slug: outcome.new_slug!,
            backlinks_updated: blUpdated,
            ...(outcome.backlinks_error ? { backlinks_error: outcome.backlinks_error } : {}),
        });
    }

    const backlinkPartialTotal = backlinksSkipped + backlinksConflicts;
    writeAdminLog(
        c,
        'bulk_move',
        `문서 대량 이동(제목 변경): "${find}" → "${replace}" (${moved.length}/${ids.length}건 이동, 역링크 ${backlinksUpdated}건 갱신` +
            `${backlinkErrors.length ? `, 역링크 실패 ${backlinkErrors.length}건` : ''}` +
            `${backlinkPartialTotal ? `, 역링크 미갱신 ${backlinkPartialTotal}건` : ''})`,
        currentUser.id
    );
    ...
    return c.json({
        ...
        backlink_error_count: backlinkErrors.length,
        // 역링크 호출은 성공했으나 일부 소스만 갱신 못 한 건(캡 초과/낙관적 락 충돌/R2 읽기 실패).
        backlinks_skipped: backlinksSkipped,
        backlinks_conflicts: backlinksConflicts,
        results: moved,
        skipped,
        backlink_errors: backlinkErrors,
        backlink_partials: backlinkPartials,
    });
});
